...
镜像由 CI 预先构建并发布，**无需在 NAS 上本地编译**，拉取即用：

```bash
# 1. 克隆项目（仅需 compose 与配置文件）
...
# 3. 拉取镜像并启动（共 3 个容器：postgres + backend + nginx 网关）
docker compose pull
docker compose up -d
...
> **本地开发/源码编译**：使用 `docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build` 从源码现场构建。
