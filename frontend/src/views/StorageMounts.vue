<template>
  <div class="page-container storage-mounts-page">
    <!-- 回到顶部 -->
    ...
    <div class="page-header">
      <div class="header-left">
        <h2 class="page-title">存储挂载点管理</h2>
        <span class="subtitle">管理下载目录和媒体库目录，支持多盘聚合与硬链接优化</span>
      </div>
    ...
    <el-card class="main-card">
      <el-tabs v-model="activeTab">
              ...
              >
                ...
                <div v-if="device.downloadMounts.length > 0" class="mount-section">
                  <h4 class="section-title">
                    ...
                    下载挂载点 ({{ device.downloadMounts.length }})
                  </h4>
                ...
                <div v-if="device.libraryMounts.length > 0" class="mount-section">
                  <h4 class="section-title">
                    ...
                    媒体库挂载点 ({{ device.libraryMounts.length }})
                  </h4>
                  <MountTable :mounts="device.libraryMounts" show-category @update="handleUpdateMount" @refresh="handleRefreshMount" @edit="handleEditMount" @delete="handleDeleteMount" />
                </div>
              </el-collapse-item>
            </el-collapse>
          </div>
        ...
        <el-tab-pane label="按类型查看" name="by-type">
          <div v-loading="loading" class="tab-content">
            ...
            <div class="type-section">
              ...
              <MountTable :mounts="libraryMounts" show-category @update="handleUpdateMount" @refresh="handleRefreshMount" @edit="handleEditMount" @delete="handleDeleteMount" />
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="所有挂载点" name="all">
          <div v-loading="loading" class="tab-content">
            <MountTable :mounts="mounts" show-category @update="handleUpdateMount" @refresh="handleRefreshMount" @edit="handleEditMount" @delete="handleDeleteMount" />
          </div>
        </el-tab-pane>
      </el-tabs>
    </el-card>
...
const groupedByDevice = computed(() => {
  ...
  function getVolumeName(hostPath?: string): string {
    if (!hostPath) return 'unknown'
    // 匹配 /volumeX/ 或 /volumeX 格式
    const match = hostPath.match(/^\/(volume\d+)/)
